...
{
    std::fstream istr(filename.data());

    std::cout << " before pass " << filename.data() <<std::endl;
    if(!istr)
        return;

    char path[256];

    std::strcpy(path, filename.data());

    std::cout << " here pass" << std::endl;
    ...
    std::cout << "loading a new material" << std::endl;
    while(istr.getline(line,256))
    {
        istringstream newLine(line, istringstream::in);
        ...
        else if (firstWord == "map_kA")
        {
            ...
            newLine >> filename;
            char *tgaName=&(string(path)+ filename)[0];
            Texture *map = new Texture(tgaName);
    ...
    std::cout << " material loaded correctly"<< std::endl;
}
